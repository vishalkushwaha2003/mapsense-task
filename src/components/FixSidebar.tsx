...
const FixSidebar: React.FC<FixSidebarProps> = ({ handleClickedFixSidebarItem }) => {
  ...
  return (
    <div className="h-[100vh] bg-slate-200 w-14  border-r border-slate-700 z-10 flex flex-col gap-6">
      <div className="mx-auto h-10 w-10 mt-2 rounded-full border border-slate-800 flex justify-center items-center">logo</div>
      
      <div className='text-center flex flex-col gap-1 '>
        <div className={`hover:cursor-pointer transition-all duration-100 ${isClickedItem==='Layers'?'bg-slate-300':'hover:bg-slate-300/50'} py-1`} onClick={()=>handleClick('Layers')}>
          <LayersOutlinedIcon fontSize='large' />
          <div className=' text-[12px] mt-[0px] '>Layers</div>
        </div>
        <div className={`hover:cursor-pointer transition-all duration-100 ${isClickedItem==='Settings'?'bg-slate-300':'hover:bg-slate-300/50'} py-1`} onClick={()=>handleClick('Settings')}>
